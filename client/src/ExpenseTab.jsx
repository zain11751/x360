...
export default function ExpenseTab({ apiBase, authHeaders, stores, customOptions, selectedStoreIds, canEdit }) {
  const [rows, setRows] = useState([]);
  const [filters, setFilters] = useState({ store_id: '', date_from: '', date_to: '', va_team: '', linked: '' });
  const [showForm, setShowForm] = useState(false);
  const [marketOrders, setMarketOrders] = useState([]);
  const [supplierOrders, setSupplierOrders] = useState([]);
  const [form, setForm] = useState({ invoice_id: '', vendor_name: '', invoice_url: '', expense_date: '', amount: 0, description: '', linked_order_id: '', linked_supplier_order_id: '', va_team: '', store_id: '' });
  ...
  const load = async () => {
    ...
    const res = await fetch(`${apiBase}/expenses?${params.toString()}`, { headers: authHeaders() });
  ...
  const openNew = async () => {
    setForm({ invoice_id: '', vendor_name: '', invoice_url: '', expense_date: '', amount: 0, description: '', linked_order_id: '', linked_supplier_order_id: '', va_team: '', store_id: selectedStoreIds[0] || stores[0]?.id || '' });
  ...
  const save = async (e) => {
    ...
    const res = await fetch(`${apiBase}/expenses`, { method: 'POST', headers: authHeaders(), body: JSON.stringify(payload) });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        {canEdit && <button onClick={openNew} className="flex items-center gap-1 px-3 py-1.5 bg-emerald-600 text-white rounded text-sm font-semibold hover:bg-emerald-700"><Plus size={16}/>New Expense</button>}
      </div>
      ...
      <div className="overflow-x-auto border border-gray-200 rounded">
        <table className="min-w-full text-xs">
          <thead className="bg-gray-50">
            <tr>{['Date','Invoice ID','Vendor','Amount','VA Team','Linked','Description'].map(h => <th key={h} className="px-3 py-2 text-left font-bold text-gray-500 uppercase">{h}</th>)}</tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {rows.map(r => (
              <tr key={r.id} className="hover:bg-gray-50">
                <td className="px-3 py-2">{r.expense_date}</td>
                <td className="px-3 py-2">{r.invoice_id}</td>
            ...
            {rows.length === 0 && <tr><td colSpan={7} className="px-3 py-6 text-center text-gray-400">No expenses found.</td></tr>}
          </tbody>
        </table>
      </div>
          ...
          <div className="bg-white rounded-lg shadow-2xl p-6 max-w-xl w-full max-h-[90vh] overflow-y-auto space-y-3">
            ...
            <form onSubmit={save} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-xs font-bold text-gray-500 uppercase">Invoice ID</label><input required value={form.invoice_id} onChange={e => setForm({...form, invoice_id: e.target.value})} className="w-full border rounded px-2 py-1.5 text-sm"/></div>
                <div><label className="text-xs font-bold text-gray-500 uppercase">Vendor Name</label><input required value={form.vendor_name} onChange={e => setForm({...form, vendor_name: e.target.value})} className="w-full border rounded px-2 py-1.5 text-sm"/></div>
                <div><label className="text-xs font-bold text-gray-500 uppercase">Expense Date</label><input type="date" required value={form.expense_date} onChange={e => setForm({...form, expense_date: e.target.value})} className="w-full border rounded px-2 py-1.5 text-sm"/></div>
